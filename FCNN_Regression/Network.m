classdef Network < handle
    ...
    methods
        ...
        function A = feedforward(obj, A)
            ...
            for i = 1 : length(obj.sizes) - 2
                A = A*obj.keep_prop;
                A = sigmoid(obj.weights{i}*A + obj.biases{i});
            end
            % The last layer is the linear neuron for regression problem.
            A = obj.weights{end}*A + obj.biases{end};
        end
        
        function [evaluation_cost, training_cost] = ...
            ...
            evaluation_cost = zeros(epochs, 1);
            ...
            for i = 1 : epochs
                ...
                t2 = clock;
                ela = etime(t2, t1);
                eva_cost = obj.total_cost(te_input, te_output, oPS);
                mse = eva_cost;
                fprintf('Epochs %d, MSE: %d, elapsed time: %5.2fs\n', i, mse, ela);
                
                if monitor_training_cost
                    train_cost = obj.total_cost(tr_input, tr_output, oPS);
                    training_cost(i) = train_cost;
                end
                if monitor_evaluation_cost
                    evaluation_cost(i) = eva_cost;
                end
                ...
                if early_stopping_n > 0
                    ...
                    if (no_cost_change == early_stopping_n)
                        evaluation_cost = evaluation_cost(1:i);
        ...
        function [] = update_parameters(obj, X, Y, eta, lmbda, n, reg_fun)
            ...
            elseif obj.optimization == "AdamS"
                obj.update_mini_batch_AdamS(X, Y, eta, lmbda, n, reg_fun)
            else
        ...
        function [] = update_mini_batch_AdamS(obj, X, Y, eta, lmbda, ~, reg_fun)
            
            % Updata the weights and the biases by AdamS(Adaptive moment estimaztion with stable weight decay). 
            ...
            if reg_fun == "L2"
                for j = 1 : length(obj.sizes) - 1
                    ...
                    weight_v_corrected_mean = mean(weight_v_corrected);
                    
                    obj.weights{1, j} = obj.weights{1, j} - (eta * weight_m_corrected ./ (sqrt(weight_v_corrected) + obj.epsilon) + eta*lmbda*obj.weights{1, j}/sqrt(weight_v_corrected_mean));
                    obj.biases{1, j} = obj.biases{1, j} - eta * biases_m_corrected ./ (sqrt(biases_v_corrected) + obj.epsilon);
                end
            elseif reg_fun == "L1"
                for j = 1 : length(obj.sizes) - 1
                    ...
                    obj.weights{1, j} = obj.weights{1, j} - (eta * weight_m_corrected ./ (sqrt(weight_v_corrected) + obj.epsilon) + eta*lmbda*sign(obj.weights{1, j})/sqrt(weight_v_corrected_mean));
                    obj.biases{1, j} = obj.biases{1, j} - eta * biases_m_corrected ./ (sqrt(biases_v_corrected) + obj.epsilon);
                end
            elseif reg_fun == "Max-norm"
                for j = 1 : length(obj.sizes) - 1
        ...
        function [nabla_B, nabla_W] = backprop(obj, X, Y)
            ...
            % The output layer is unnecessary to be dropouted and the output neuron is identity neuron.
        ...
        function [nabla_B, nabla_W] = grad_cal(obj, X, Y, weight_ahead, bia_ahead)
            ...
            % The output neuron is identity neuron.
            z = obj.weights{1, end} * activation + repmat(obj.biases{1, end}, 1 , mini_batch_size);
            zs{length(obj.sizes) - 1} = z;
            activation = z;
            activations{length(obj.sizes)} = activation;
        ...
        function cost = total_cost(obj, X, Y, oPS)
            
            predict = mapminmax('reverse', obj.feedforward(X), oPS);
            expect = mapminmax('reverse', Y, oPS);
            
            n = size(X, 2);
            cost = sum(obj.cost.fn(predict, expect)) / n;
        end
